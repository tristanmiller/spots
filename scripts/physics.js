...
function buildDiffusionMatrix (elms, diff) {
    let a = diff*TIME_STEP*elms.length/elms[0].elm_length;
    let A = [];
...
function diffuse (elms, quantity, diff) {
  ...
  for (let k = 0; k < 10; k++) {
    for (let i = 0, l = elms.length; i < l; i++) {
      let quantity_new = elms[i][quantity];
      if(i > 0) {
          quantity_new -= A[i][i - 1]*new_quantities[i - 1];
      }
      if(i < l - 1) {
          quantity_new -= A[i][i + 1]*new_quantities[i + 1];
      }
      quantity_new = quantity_new/A[i][i];
      new_quantities[i] = quantity_new;
    }
  }
  return new_quantities;
}

function advect (elms, quantity) {
  let new_quantities = [];
  for (let i = 0, l = elms.length; i < l; i++) {
    ...
    let new_quantity = ip[0][quantity] + frac*(ip[1][quantity] - ip[0][quantity]);
    new_quantities.push(new_quantity);
...
function visualise() {
  ...
  for (let i = 0, l = elm_list.length; i < l; i++) {
    ...
    elm_divs[i].innerHTML =  Math.floor(elm.pressure)/1000 + 'kPa <br>'+ Math.floor(1000*elm.velocity)/1000 + 'm/s';
  }
